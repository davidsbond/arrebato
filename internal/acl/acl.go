...
func Normalize(ctx context.Context, a *acl.ACL) (*acl.ACL, error) {
	...
	err := tracing.WithinSpan(ctx, "ACL.Normalize", func(ctx context.Context, span trace.Span) error {
		for i, entry := range a.GetEntries() {
			...
			case entry.GetClient() == "":
				return fmt.Errorf("%w: invalid entry %v: client name must be provided", ErrInvalidACL, i)
			case entry.GetTopic() == "":
				return fmt.Errorf("%w: invalid entry %v: topic name must be provided", ErrInvalidACL, i)
			case len(entry.GetPermissions()) == 0:
				return fmt.Errorf("%w: invalid entry %v: at least one permission is required", ErrInvalidACL, i)
			}
			...
			for _, permission := range entry.GetPermissions() {
				...
				return fmt.Errorf("%w: invalid permission in entry %v: %s", ErrInvalidACL, i, permission)
			}
